...
function findIndexOf(data: any, callback: any) {
  let idx = -1
...
function findContainer(data: any, modal: any) {
  return findIndexOf(data, (item: any) => item.modals.indexOf(modal) !== -1)
}
...
function setContainerStyle(data: any, container: any) {
  ...
  if (data.overflowing) {
    ...
    const fixedNodes: any = document.querySelectorAll('.mui-fixed')
...
function removeContainerStyle(data: any, container: any) {
  Object.keys(data.style).forEach((key: any) => {
 ...
 */
class ModalManager {
  public hideSiblingNodes: any
  public handleContainerOverflow: any
  public modals: any
  ...
  public add = (modal: any, container: any) => {
